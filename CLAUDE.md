...
| `src/log.h/cpp` | `logging` / 非同期ログ (ダブルバッファ + バックグラウンドスレッド) |
...
| General | InitDelayMs | 5000 | UE4 初期化待機 (ms) |
| General | DemoMode | true | false = 実チャット駆動 |
...
| Translation | Mode | "Japanese" | Off / Japanese / Zundamon / English / Russian / Chinese / Korean |
| TTS | VoicevoxStyleId | 3 | VOICEVOX スタイル ID (3 = ずんだもんノーマル、JAZ モード) |
| TTS | VoicevoxJaStyleId | 13 | VOICEVOX スタイル ID (13 = 青山龍星ノーマル、JA モード) |
| TTS | Volume | 0.8 | 音量 (0.0〜1.0) |
| TTS | Mode | "Translated" | Off / Original / Translated (読み上げ対象) |
| TTS | VerboseLog | false | 合成フェーズの詳細ログ |
...
version.dll (永続)
  ...
  ├─ ProcessEvent フック: vtable[66] ハードコード、最大 64 枠 (MinHook)
  ├─ DX11 Present フック: vtable[8]
  ├─ DX11 ResizeBuffers フック: vtable[13] (ウィンドウリサイズ時の RTV 解放)
...
| Medium | phi4-mini | 512 | 0 | 0.1 | 120 |
...
1. VOICEVOX Core — TranslationMode が JA (青山龍星) または JAZ (ずんだもん) かつ初期化済み
2. Sherpa-ONNX — その他言語 / VOICEVOX 未初期化時のフォールバック
3. JA Supertonic-3 フォールバック — 対象言語モデル未インストール時 (多言語対応)
...
- TranslationMode=JAZ + VOICEVOX 初期化成功 → VOICEVOX (style_id=3 = ずんだもんノーマル)
- TranslationMode=JA + VOICEVOX 初期化成功 → VOICEVOX (style_id=13 = 青山龍星ノーマル、15.vvm 必須)
- VOICEVOX 失敗または非 JA/JAZ → Sherpa-ONNX にフォールバック
...
### espeak-ng VITS モデルのクラッシュ

EN / RU / KO の VITS (Piper/mimic3) モデルは espeak-ng を必要とし、Create/Generate 両フェーズで `abort()` によりクラッシュする (SEH では捕捉不可)。
実装ではこれらを事前にスキップし、JA Supertonic-3 (多言語対応) でフォールバックする。
`lang != ZH` の条件で ZH (中国語、espeak-ng 不要) のみ VITS を使用可能。
...
### ProcessEvent vtable インデックス

`dllmain.cpp` で `constexpr int peIndex = 66` にハードコード。config.ini からは読み込まない。
ゲームアップデートでインデックスが変わった場合はソースを直接変更してリビルドが必要。
